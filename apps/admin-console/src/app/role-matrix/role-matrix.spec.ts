...
describe('RoleMatrix', () => {
  ...
  it('shows a resolvable role as selectable and distinguishes its informational display name', async () => {
    ...
    const searchPromise = fixture.componentInstance.searchRoles();
    httpMock.expectOne('/api/ads/internal/directory/roles?query=doctor').flush({
      items: [{ canonicalId: 'kc:tenant-a:realm:doctor', externalId: 'doctor', name: 'Doctor', description: '' }],
    });
    await searchPromise;
    fixture.detectChanges();
    ...
    expect(option.textContent).toContain('persists as kc:tenant-a:realm:doctor');
  });
  ...
  it('loads the full matrix for a selected role and renders its current grants', async () => {
    ...
    const selectPromise = fixture.componentInstance.selectRole({
      canonicalId: 'kc:tenant-a:realm:doctor', externalId: 'doctor', name: 'Doctor', description: '',
    });
    httpMock
      .expectOne('/api/admin/authz/tenants/tenant-a/roles/kc:tenant-a:realm:doctor/permissions')
      .flush({
        permissions: [
