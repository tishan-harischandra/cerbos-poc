...
## Organization-scoped SSO requires Keycloak's native Organization flow (issue #84)

The original issue #84 spike used a custom `browser-with-org-selector` flow
that omitted Keycloak's native Organization alternative subflow. With that
configuration, an existing SSO cookie plus `prompt=none` and a different
`organization:<alias>` scope returned `error=login_required`. The hidden-frame
browser implementation swallowed that refusal and left the visible hospital
unchanged.

The stock Keycloak 26.7.3 browser flow provided the missing comparison: it
places its native Organization branch before the forms branch. The tenant
fixtures now preserve that structure inside the custom browser flow while
retaining the custom initial selector after credentials.

Measured again with `scripts/tests/hospital-switch-e2e.sh` against the real TLS
stack and `user-doctor-multi`'s North and South memberships:

| Question | Result |
|---|---|
| Does the initial authorization still produce a North Hospital token? | **Yes.** The identity-first native step and custom selector complete before the code exchange. |
| Does the existing SSO session satisfy `prompt=none` for South Hospital? | **Yes.** Keycloak returns an authorization code without presenting credentials. |
| Does exchanging that code with its PKCE verifier produce the requested hospital? | **Yes.** The access token contains exactly `organization: ["south-hospital"]`. |
| Can the same session request an organization it does not belong to? | **No.** Keycloak returns an OIDC error and no authorization code. |

The browser now performs the same operation as a top-level Authorization Code
+ PKCE transition. It stores only verifier, state, target alias, transition
kind, and return path; the callback validates state and requires the issued
token's active hospital to equal the pending target before committing it. The
old `login_required` result remains historical evidence of the incomplete
custom flow, not an accepted switch outcome.
