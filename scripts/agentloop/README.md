...
| `escalate.sh` | Park an issue after the cap, label `needs-human`, return to `main` |
...
- The self-review cycle is capped at `AGENTLOOP_MAX_REVIEW_ROUNDS` (default 3);
  `review-pr.sh` exits 4 once exhausted so the loop escalates instead of
  spinning.

## Exit codes

`0` success, `1` usage or tooling error, `3` nothing ready to start,
`4` review round cap exhausted.

## Environment

- `AGENTLOOP_DEFAULT_BRANCH` (default `main`)
- `AGENTLOOP_MAX_REVIEW_ROUNDS` (default `3`)
